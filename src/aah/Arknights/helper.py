...
from src.aah import config
from src.aah.imgreco import (before_operation,common,end_operation,main,task,recruit,util,stage_ocr,inventory,imgops)
from src.aah.connector import auto_connect
from src.aah.connector.ADBConnector import ADBConnector
from . import stage_path
from .frontend import DummyFrontend
from src.aah.Arknights.click_location.click_location import CLICK_LOCATION
from src.aah.Arknights.flags import *
from src.aah.util.excutil import guard

from src.aah.Arknights import frontend
...
class ArknightsHelper(object):
    ...
    def wait_for_still_image(self, threshold=16, crop=None, timeout=60, raise_for_timeout=True, check_delay=1):
        ...
        while (t1 := time.monotonic()) < ts:
            ...
            screenshot2 = shooter()
            mse = imgops.compare_mse(screenshot, screenshot2)
            if mse <= threshold:
                return screenshot2
    ...
    def operation_once_statemachine(self, c_id):
        ...
        def on_prepare(smobj):
            ...
            while True:
                ...
                recoresult = before_operation.recognize(screenshot)
            ...
            if self.CURRENT_STRENGTH < int(recoresult['consume']):
                ...
                if recoresult['consume_ap'] and self.can_perform_refill():
                    logger.info('尝试回复理智')
                    self.tap_rect(before_operation.get_start_operation_rect(self.viewport))
                    self.__wait(SMALL_WAIT)
                    screenshot = self.adb.screenshot()
                    refill_type = before_operation.check_ap_refill_type(screenshot)
                    ...
                    if confirm_refill:
                        self.tap_rect(before_operation.get_ap_refill_confirm_rect(self.viewport))
                        self.refill_count += 1
                        self.__wait(MEDIUM_WAIT)
                        return  # to on_prepare state
                    logger.error('未能回复理智')
                    self.tap_rect(before_operation.get_ap_refill_cancel_rect(self.viewport))
                raise StopIteration()

            if not recoresult['delegated']:
                logger.info('设置代理指挥')
                self.tap_rect(before_operation.get_delegate_rect(self.viewport))
                return  # to on_prepare state
        ...
        def on_troop(smobj):
            ...
            while True:
                self.__wait(TINY_WAIT, False)
                screenshot = self.adb.screenshot()
                recoresult = before_operation.check_confirm_troop_rect(screenshot)
                if recoresult:
                    logger.info('确认编队')
                    break
                else:
                    ...
                    if count_times <= 7:
                        ...
                        raise StopIteration()
            self.tap_rect(before_operation.get_confirm_troop_rect(self.viewport))
        ...
        def on_operation(smobj):
            ...
            screenshot = self.adb.screenshot()
            if end_operation.check_level_up_popup(screenshot):
                logger.info("等级提升")
            ...
            if smobj.prepare_reco['consume_ap'] and not smobj.prepare_reco['no_friendship']:
                detector = end_operation.check_end_operation
            else:
                detector = end_operation.check_end_operation_alt
            end_flag = detector(screenshot)
            if not end_flag and t > 300:
                if end_operation.check_end_operation2(screenshot):
                    self.tap_rect(end_operation.get_end2_rect(screenshot))
                    screenshot = self.adb.screenshot()
                    end_flag = end_operation.check_end_operation_alt(screenshot)
            if end_flag:
                logger.info('战斗结束')
                self.operation_time.append(t)
                crop = end_operation.get_still_check_rect(self.viewport)
                if self.wait_for_still_image(crop=crop, timeout=15, raise_for_timeout=True):
                    smobj.state = on_end_operation
                return
            dlgtype, ocrresult = common.recognize_dialog(screenshot)
            if dlgtype is not None:
                if dlgtype == 'yesno' and '代理指挥' in ocrresult:
                    ...
                    if config.get('behavior/mistaken_delegation/settle', False):
                        ...
                        self.tap_rect(common.get_dialog_right_button_rect(screenshot))
                        self.__wait(2)
                        smobj.stop = True
                        return
                    else:
                        logger.info('放弃关卡')
                        self.tap_rect(common.get_dialog_left_button_rect(screenshot))
                        # 关闭失败提示
                        self.wait_for_still_image()
                        self.tap_rect(common.get_reward_popup_dismiss_rect(screenshot))
                        # FIXME: 理智返还
                        self.__wait(1)
                        smobj.stop = True
                        return
        ...
        def on_level_up_popup(smobj):
            self.__wait(SMALL_WAIT, MANLIKE_FLAG=True)
            logger.info('关闭升级提示')
            self.tap_rect(end_operation.get_dismiss_level_up_popup_rect(self.viewport))
            self.wait_for_still_image()
            smobj.state = on_end_operation
        ...
        def on_end_operation(smobj):
            screenshot = self.adb.screenshot()
            logger.info('离开结算画面')
            self.tap_rect(end_operation.get_dismiss_end_operation_rect(self.viewport))
            try:
                # 掉落识别
                drops = end_operation.recognize(screenshot)
    ...
    def back_to_main(self):  # 回到主页
        ...
        while True:
            ...
            if main.check_main(screenshot):
                break

            # 检查是否有返回按钮
            if common.check_nav_button(screenshot):
                logger.info('发现返回按钮，点击返回')
                self.tap_rect(common.get_nav_button_back_rect(self.viewport))
                self.__wait(SMALL_WAIT)
                # 点击返回按钮之后重新检查
                continue

            if common.check_get_item_popup(screenshot):
                logger.info('当前为获得物资画面，关闭')
                self.tap_rect(common.get_reward_popup_dismiss_rect(self.viewport))
                self.__wait(SMALL_WAIT)
                continue

            # 检查是否在设置画面
            if common.check_setting_scene(screenshot):
                logger.info("当前为设置/邮件画面，返回")
                self.tap_rect(common.get_setting_back_rect(self.viewport))
                self.__wait(SMALL_WAIT)
                continue

            # 检测是否有关闭按钮
            rect, confidence = common.find_close_button(screenshot)
            if confidence > 0.9:
                logger.info("发现关闭按钮")
                self.tap_rect(rect)
                self.__wait(SMALL_WAIT)
            ...
            dlgtype, ocr = common.recognize_dialog(screenshot)
            if dlgtype == 'yesno':
                ...
                self.tap_rect(common.get_dialog_ok_button_rect(screenshot))
                self.__wait(1)
                continue
            retry_count += 1
    ...
    def clear_task(self):
        ...
        screenshot = self.adb.screenshot()
        logger.info('进入任务界面')
        self.tap_quadrilateral(main.get_task_corners(screenshot))
        self.__wait(SMALL_WAIT)
        screenshot = self.adb.screenshot()

        hasbeginner = task.check_beginners_task(screenshot)
        if hasbeginner:
            logger.info('发现见习任务，切换到每日任务')
            self.tap_rect(task.get_daily_task_rect(screenshot, hasbeginner))
            self.__wait(TINY_WAIT)
            screenshot = self.adb.screenshot()
        self.clear_task_worker()
        logger.info('切换到每周任务') #默认进入见习任务或每日任务，因此无需检测，直接切换即可
        self.tap_rect(task.get_weekly_task_rect(screenshot, hasbeginner))
        self.clear_task_worker()
    ...
    def clear_task_worker(self):
        ...
        kickoff = True
        while True:
            if common.check_nav_button(screenshot) and not task.check_collectable_reward(screenshot):
                logger.info("奖励已领取完毕")
                break
            if kickoff:
                logger.info('开始领取奖励')
                kickoff = False
            self.tap_rect(task.get_collect_reward_button_rect(self.viewport))
            screenshot = self.adb.screenshot(cached=False)

    def recruit(self):
        import src.aah.imgreco.recruit
        from . import recruit_calc
        logger.info('识别招募标签')
        tags = recruit.get_recruit_tags(self.adb.screenshot())
    ...
    def find_and_tap(self, partition, target):
        ...
        while True:
            screenshot = self.adb.screenshot()
            recoresult = map.recognize_map(screenshot, partition)
    ...
    def find_and_tap_episode_by_ocr(self, target):
        import src.aah.imgreco.stage_ocr
        from src.aah.resources.imgreco.map_vectors import ep2region, region2ep
        target_region = ep2region.get(target)
        if target_region is None:
            logger.error(f'未能定位章节区域, target: {target}')
            raise RuntimeError('recognition failed')
        vw, vh = util.get_vwvh(self.viewport)
    ...
    def find_and_tap_stage_by_ocr(self, partition, target, partition_map=None):
        ...
        if partition_map is None:
            from src.aah.resources.imgreco.map_vectors import stage_maps_linear
            partition_map = stage_maps_linear[partition]
        target_index = partition_map.index(target)
        while True:
            screenshot = self.adb.screenshot()
            tags_map = stage_ocr.recognize_all_screen_stage_tags(screenshot)
    ...
    def find_and_tap_daily(self, partition, target, *, recursion=0):
        screenshot = self.adb.screenshot()
        recoresult = map.recognize_daily_menu(screenshot, partition)
        if target in recoresult:
    ...
    def goto_stage_by_ocr(self, stage):
        ...
        self.back_to_main()
        logger.info('进入作战')
        self.tap_quadrilateral(main.get_ballte_corners(self.adb.screenshot()))
        self.__wait(TINY_WAIT)
        if path[0] == 'main':
            vw, vh = util.get_vwvh(self.viewport)
            ...
            logger.info('选择类别')
            self.tap_rect(map.get_daily_menu_entry(self.viewport, path[0]))
    ...
    def get_credit(self):
        ...
        screenshot = self.adb.screenshot()
        logger.info('进入好友列表')
        self.tap_quadrilateral(main.get_friend_corners(screenshot))
        self.__wait(SMALL_WAIT)
        self.tap_quadrilateral(main.get_friend_list(screenshot))
        self.__wait(SMALL_WAIT)
        logger.info('访问好友基建')
        self.tap_quadrilateral(main.get_friend_build(screenshot))
        self.__wait(MEDIUM_WAIT)
        building_count = 0
        while building_count <= 11:
            screenshot = self.adb.screenshot()
            self.tap_quadrilateral(main.get_next_friend_build(screenshot))
            self.__wait(MEDIUM_WAIT)
    ...
    def get_building(self):
        ...
        screenshot = self.adb.screenshot()
        logger.info('进入我的基建')
        self.tap_quadrilateral(main.get_back_my_build(screenshot))
        self.__wait(MEDIUM_WAIT + 3)
        self.tap_quadrilateral(main.get_my_build_task(screenshot))
        self.__wait(SMALL_WAIT)
        logger.info('收取制造产物')
        self.tap_quadrilateral(main.get_my_build_task_clear(screenshot))
        self.__wait(SMALL_WAIT)
        logger.info('清理贸易订单')
        self.tap_quadrilateral(main.get_my_sell_task_1(screenshot))
        self.__wait(SMALL_WAIT + 1)
        self.tap_quadrilateral(main.get_my_sell_tasklist(screenshot))
        self.__wait(SMALL_WAIT -1 )
        sell_count = 0
        while sell_count <= 6:
            screenshot = self.adb.screenshot()
            self.tap_quadrilateral(main.get_my_sell_task_main(screenshot))
            self.__wait(TINY_WAIT)
            sell_count = sell_count + 1
        self.tap_quadrilateral(main.get_my_sell_task_2(screenshot))
        self.__wait(SMALL_WAIT - 1)
        sell_count = 0
        while sell_count <= 6:
            screenshot = self.adb.screenshot()
    ...
    def get_inventory_items(self):
        import src.aah.imgreco.inventory
        all_items_map = {}

        self.back_to_main()
        logger.info("进入仓库")
        self.tap_rect(inventory.get_inventory_rect(self.viewport))
        ...
        while True:
            ...
            self.__swipe_screen(move)
            screen_items_map = inventory.get_all_item_in_screen(screenshot)
            ...
            if record['type'] == 'tap':
                ...
                for _ in range(repeat):
                    if mode == 'match_template':
                        ...
                        (x, y), r = imgops.match_template(gray_screen, template)
                        x = x // ratio
                        y = y // ratio
